...
# New Manually Validated Data 
In the additional analysis, we received two excel files `manually validated spy vs 5fold proteins for VL 9-12-14.xlsx` and `H2AFY all peptides 9-10-14 A.xlsx` with manually validated data(New Data 2). Data from these files were used to replace the corresponding data from `Spy vs 5 FOLD all data for King Jordan 5-14-13 AP.csv`. The resulting file `Merge.DataSets.20141001.xlsx` with the merged data was then exported to `Merge.DataSets.20141001.csv` to run the same analysis as we previously did for the old data set `Spy vs 5 FOLD all data for King Jordan 5-14-13 AP.csv` in the directory `May.14.Proteomics.data from Amol`.

Before the analysis, the data file was checked:

**1. Are the expression values different for the same peptide? **

No, they are different ??? Replace the peptide expression value with the expression values from the new data 2.

**2. Are the peptide assignments different(i.e. the same peptide was assigned to different proteins)? **

No, they are slightly different. For a given protein from the  validation, the assigned peptides are the same as or a subset of the old data set ??? Adopt the peptide assignment from new data 2
    - The resulting merged dataset will have less peptides than the old dataset
    - The peptides which were no longer assigned to the same protein in the new data 2 will be discarded

In the following analysis, the new merged data set will be analyzed as a whole by

- Performing the same normalization as we did on the old data set
- Comparing the before and after normalization of the merged data set
...
```{r readRawData2, cache = TRUE}
tb <- read.csv("Merge.DataSets.20141001.csv", skip = 1, header = TRUE)
str(tb)
```
...
res1 <- getSymbol(prot$id)
```

```{r showSymbol2}
head(res1, n = 2)
mdf <- merge( prot, res1, by.x = "id", all.x = TRUE, by.y = "refseq_peptide")
mdf <- mdf[, c(1, 9, 2:7, 8)]
head(mdf, n = 2)
prot <- mdf
rm(mdf)
...
```{r densityBeforNorm2, warning = FALSE}
library(ggplot2)
...
```{r plotNormalized2, warning = FALSE}
#Boxplot
...
```{r saveBoxplotAfter2, eval = FALSE, cache = FALSE}
...
```{r saveDensityAfter2, eval = FALSE, cache = FALSE}
...
```{r diffExp2, message = FALSE, warning = FALSE, fig.height = 8, fig.width = 8}
library(oompaBase)
...
```{r zTransformation2, fig.height = 5, fig.width = 5}
library(gplots)
...
## Raw Expression of Individual Protein before Normalization
...
```{r topCV2, fig.height = 10, fig.width = 10}
top.n <- 50
...
```{r checkIndividualProtein2, fig.height = 5, fig.width = 7}
